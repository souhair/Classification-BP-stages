function processed_data = process_pulse_wave(ppg_signals, pulse_rates, ages, fs)
    ...
    for i = 1:length(ppg_signals)
        ...
        % Convert to step representation
        [step_amps, step_times, step_durations] = convert_to_step_representation(ppg_signals{i}, pulse_rates(i), fs);
...
%         result.NumOfPeaks= length(result.peaks);
        ...
        % Process each cardiac cycle using the processing functions
