function features = extract_pulse_wave_features(ppg_signals, pulse_rates, ages, fs)
    % PULSE WAVE FEATURE EXTRACTION 

    fprintf('=== PULSE WAVE FEATURE EXTRACTION ===\n\n');
...
function nonlinear = extract_nonlinear_features_single(ppg_signal)
    % COMPLETELY REWRITTEN - Robust nonlinear feature extraction
    nonlinear = struct();
...
function lyapunov_exp = compute_robust_lyapunov(signal)
    ...
    try
        % Simple divergence-based approximation
...
function feat = create_complete_feature_structure(subject_id, age, p_coefficient)
    ...
    feat.engineered = create_default_engineered_structure(); % NEW

end
function basic = create_default_basic_structure()
    % Create default basic demographic structure
    basic = struct();
    basic.age = 0;
    basic.gender = '';
    basic.height = 0;
...
function morphology = create_default_morphology_structure()
    morphology = struct();
...
function func_feat = create_default_function_structure()
    func_feat = struct();
    func_feat.F_mean = 0;
    func_feat.F_std = 0;
    func_feat.F_cv = 0;
...
function temporal = create_default_temporal_structure()
    temporal = struct();
    temporal.total_duration = 0;
    temporal.mean_cycle_duration = 0;
    temporal.cycle_duration_std = 0;
